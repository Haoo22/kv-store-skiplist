...
- 比较对象：`skiplist_sharded` vs `std_map_mutex`
- 场景：预加载后、读多写少
- 结论：细粒度锁跳表已经明显超过原版红黑树基线

可以直接引用：

- `preload=100000`：`2.77x`
- `preload=300000`：`2.90x`

同时主动补一句边界：

- 这是“细粒度锁跳表”对“原版红黑树基线”的比较
- 它对应的是开题报告里“读写锁进一步优化”的实验性落地，不是主线服务端已经改成分片并行执行
...
- 如果按答辩主比较口径，细粒度锁跳表已经在读多写少场景下明显超过原版红黑树基线
